...
int main()
{
  ...
  string projectName = "exampleAdvectionDiffusion";
  ...
  double xMin = -M_PI_2, xMax = M_PI_2,
         yMin = -M_PI_2, yMax = M_PI_2;

  CaslGrid2D grid(xMin, xMax, yMin, yMax, nX, nY);

  double initialT = 0.0, finalT = 3.0, current_time = initialT;
  double dt;
  ...
  for (int i = 1; i <= nX; i++)
  {
    for (int j = 1; j <= nY; j++)
    {
      c1(i, j) = -1.0;
      c2(i, j) = 0.0;
      c1(i, j) = 2 * -cos(grid.x(i)) * sin(grid.y(j));
      c2(i, j) = 2 *  sin(grid.x(i)) * cos(grid.y(j));
    }
  }
...
for (int i = 1; i <= nX; ++i)
    for (int j = 1; j <= nY; ++j)
    {
      if (pow(grid.x(i) - 1, 2) + pow(grid.y(j), 2) < 0.0625) phi_n(i, j) = 1;
      else phi_n(i, j) = 0; 
    }
