/**
 * @file main.cpp
 * @brief Normal Advection Equation Solver using CASL-HJX Framework
 *
 * MATHEMATICAL MODEL:
 * ∂φ/∂t + vₙ|∇φ| = 0
 *
 ...
 * - vₙ is the advection velocity component (constant)
 * - Domain: [-2, 2] × [-2, 2] with linear extrapolation
 * - Initial condition: φ(x,y,0) = 1 - x⁴ - y⁴ + 3²
 *
 * NUMERICAL METHODS:
 ...
 * - CFL condition: dt ≤ 0.4 * min(dx/|vₙ|, dy/|vₙ|, dx, dy)
 *
 * PHYSICAL SIGNIFICANCE:
 * Advection in the normal direction is useful for many applications in level-set methods.
 *
 * OUTPUT FORMAT:
...
#include "projectNormalAdvection_lib/CaslHamiltonianNormalAdvection2D.h"
...
/**
 * @brief Main solver routine for normal advection equation
 *
...
int main()
{
    ...
    cout << "CASL-HJX: Normal Advection Equation Solver" << endl;
    cout << "==========================================" << endl;
    ...
    // vₙ = 1 (uniform transport in normal direction)
    CaslArray2D<double> vn(nX, nY);
    for (int i = 1; i <= nX; ++i) {
        for (int j = 1; j <= nY; ++j) {
            vn(i, j) = 1; // velocity of 1 in normal direction
        }
    }

    cout << "Velocity field: vₙ = " << vn(1,1) << " (uniform)" << endl;

    // Set initial condition: φ(x,y,0) = 1 - x⁴ - y⁴ + 3²
    // This gives a level set that changes over time
    for (int i = 1; i <= nX; ++i) {
        for (int j = 1; j <= nY; ++j) {
            phiN(i, j) = 1 - pow(grid.x(i), 4.) - pow(grid.y(j), 4.) + 3 * pow(grid.x(i), 2.);
        }
    }

    cout << "φ(x,y,0) = 1 - x⁴ - y⁴ + 3²" << endl;

    // ====================================================================
    // SECTION 5: NUMERICAL SOLVER CONFIGURATION
    // ====================================================================

    // Configure Hamiltonian for normal advection
    CaslHamiltonianNormalAdvection2D hamiltonian(grid, vn);
    ...
    double dt = 0.4 * min({dx / max(vn.maxAbs(), 1e-12),
                           dy / max(vn.maxAbs(), 1e-12),
                           dx, dy});
     ...
     *
     * Where L(φ) = -vₙ|∇φ| (advection operator)
     */
